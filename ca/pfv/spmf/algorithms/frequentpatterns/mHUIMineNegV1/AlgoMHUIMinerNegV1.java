package ca.pfv.spmf.algorithms.frequentpatterns.mHUIMineNegV1;
...
public class AlgoMHUIMinerNegV1 {
	...
	public void runAlgorithm(String input, String output, int minimumUtility) throws IOException {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				// revised transaction in ascending order of RTWU
	...
	/**
	 * Method to compare items by their RTWU
	 * @param item1 an item
	...
	private int compareItemsAsc(int item1, int item2, Map<Integer, Integer> mapItemEstimatedUtility) {
		...
		// if the same, use the lexical order otherwise use the RTWU
